...
import (
	...
	PumpData "heatPumpV2/heatPumpV2"
	PumpController "heatPumpV2/pumpController"
	websocket "heatpumpV2/webSocket"
	"log"
	"net/http"
	"net/smtp"
	"strconv"
...
The heat pump inverter has gone off line and attempting recover by cycling the power to it did not bring it back!`))
		if err != nil {
	...
	for {
		...
		case <-ticker.C:
			...
			if !lastHeatPumpData.Coil[InverterPowerAlarm-1] {
				...
				// If the alrm was first seen more than 1 minute ago then cycle the power to the inverter.
...
func webGetValues(w http.ResponseWriter, _ *http.Request) {
  ...
  <body>
	...
	_, err = fmt.Fprint(w, `
        ...
        <br />
			<button class="frmSubmit" type="text" onclick="sendFormData('pumpForm', 'setHoldingRegisters')">Submit Changes</button>&nbsp;&nbsp;
    ...
    <div id="hpcoils">
      <form onsubmit="return false;" id="heatPumpForm">
        <button class="frmSubmit" type="text" onclick="sendFormData('heatPumpForm', 'setHoldingRegisters')">Submit Changes</button>&nbsp;&nbsp;
		<button class="frmSubmit" type="button" id="hpOnOff" onclick="startHeatPump()">Start Heat Pump</button><br />
...
func main() {
	...
	db, err := sql.Open("mysql", "pi:7444561@tcp(aberhome1.local:3306)/logging")
